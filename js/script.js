    ...
    searchInput.addEventListener("keydown", function (event) {
        ...
        if (event.key === "Enter" && searchInputValue != "") {
            const filteredItems = recipeData.filter(
                (item) =>
                    item.other_categories.some(
                        (category) => category.toLowerCase() === searchInputValue
                    ) ||
                    ...
                    item.ingredients.some((ingredient) =>
                        ingredient.ingredient_name.toLowerCase().includes(searchInputValue)
                    )
            );
...
function filterFunction(recipeData) {
    ...
    submitFilter.addEventListener("click", function () {
        ...
        if (selectedFilters.length != 0) {
            const filteredItems = recipeData.filter(
                (item) =>
                    item.other_categories.some((category) =>
                        selectedFilters.some((filter) => category.toLowerCase().includes(filter))
                    ) ||
                    selectedFilters.some((filter) =>
                        item.cuisine_type.toLowerCase().includes(filter)
                    ) ||
                    selectedFilters.some((filter) =>
                        item.cooking_skill_level.toLowerCase().includes(filter)
                    )
            );
...
function generateDailyRecipe(recipeData) {
    ...
    if (
        dateToday !== localStorage.getItem("dateToday") ||
        !localStorage.getItem("dailyRecipe") ||
        !localStorage.getItem("dailyRecipeBreakfast")
    ) {
        localStorage.setItem("dateToday", dateToday);
        const recipeDataNoBreakfast = recipeData.filter(
            (recipe) =>
                !recipe.other_categories.some((category) => category.toLowerCase() === "breakfast")
        );
        const recipeBreakfast = recipeData.filter((recipe) =>
            recipe.other_categories.some((category) => category.toLowerCase() === "breakfast")
        );
        ...
        localStorage.setItem(
            "dailyRecipeBreakfast",
            JSON.stringify(randomDailyBreakfast.slice(0, 3))
        );
    }
}
...
function updateDuplicateCardInformation(cardClone, number, data, section) {
    ...
    const recipeTitle = document
        .getElementById(`recipe-title-${section}-${number}`)
        .querySelector("a");
...
function bookmarkInteraction(section, recipeData) {
    ...
    bookmarkButton.forEach((bookmarkButton, index) => {
        ...
        const recipeTitle = document
            .getElementById(`recipe-title-${section}-${index + 1}`)
            .textContent.replace(/[\n\r]+|[\s]{2,}/g, " ")
            .trim();
...
function removeBookmark(recipeTitle) {
    ...
    const updatedBookmark = bookmarksList.filter(
        (bookmark) => bookmark.recipe_title !== recipeTitle
    );
